      ...
      tf_listener_(std::make_shared<tf2_ros::TransformListener>(*tf_buffer_))
{
    ...
    costmap_pub_ = this->create_publisher<nav_msgs::msg::OccupancyGrid>("costmap", 30);
}
...
void OccMapToCostmapNode::occMapCallback(const nav_msgs::msg::OccupancyGrid::SharedPtr msg)
{
    ...
    clearCenterCosts(costmap);
    publishCostmap();
}

void OccMapToCostmapNode::clearCenterCosts(nav2_costmap_2d::Costmap2D* costmap)
{
    unsigned int size_x = costmap->getSizeInCellsX();
    unsigned int size_y = costmap->getSizeInCellsY();
    
    // Define the radius for the circular region
    double radius = 8.0; // For example, 12 cells
    unsigned int center_x = size_x / 2;
    unsigned int center_y = size_y / 2;
...
void OccMapToCostmapNode::publishCostmap()
{
    ...
    costmap_msg->header.frame_id = "lidar_odom";
