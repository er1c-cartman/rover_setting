...
void ElevationMap::move(const Eigen::Vector2d& position) {
  ...
  if (rawMap_.move(position, newRegions)) {
    RCLCPP_INFO(nodeHandle_->get_logger(),"Elevation map has been moved to position (%f, %f).", rawMap_.getPosition().x(), rawMap_.getPosition().y());
...
bool ElevationMap::clean() {
  ...
  rawMap_.get("variance") =
      rawMap_.get("variance").unaryExpr(VarianceClampOperator<float>(minVariance_, maxVariance_));
