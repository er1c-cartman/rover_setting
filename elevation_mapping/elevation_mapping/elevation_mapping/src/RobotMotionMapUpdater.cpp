...
RobotMotionMapUpdater::RobotMotionMapUpdater(rclcpp::Node::SharedPtr nodeHandle) : nodeHandle_(nodeHandle), covarianceScale_(1.0) {
  previousReducedCovariance_.setZero();
  previousUpdateTime_ = nodeHandle_->now();
...
bool RobotMotionMapUpdater::readParameters() {
  nodeHandle_->declare_parameter("robot_motion_map_update/covariance_scale", 1.0);
